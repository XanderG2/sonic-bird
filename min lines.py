def main(): 
    import pygame, random, sys;pygame.init();pygame.font.init();myfont = pygame.font.SysFont("monospace", 15);white = (255,255,255);black = (0,0,0);grey = (128,128,128);yellow = (255,255,0);color = black;width = 1200;height = 800;screen = pygame.display.set_mode((width, height));pygame.display.set_caption("Flappy bird but very strange and also not really flappy bird its more like sonic because you have to collect the rings... ill call it sonic bird | Xander");highscore = 0;player = pygame.Rect(width//2,height//2,50,50);player.x -= player.width//2;player.y -= player.height//2;ring = pygame.Rect(width//2, height//2, 5, 75);clock = pygame.time.Clock();fps = 60;time_flashing = 0;lives = 3;score = -1;gravity = 0;direction = "r";running = True
    while running:
        ...
        if time_flashing < 0: color = black
        ...
        player.x = player.x+5 if direction == "r" else player.x - 5
